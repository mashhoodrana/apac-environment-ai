...
export default function Temperature() {
  ...
  useEffect(() => {
    if (result && result.temperature_data) {
      // Process historical data for the chart
      const labels = result.temperature_data.dates || [];
      const data = result.temperature_data.values || [];
      ...
      if (result.temperature_prediction) {
        const predictionLabels = result.temperature_prediction.dates || [];
        const predictionValues = result.temperature_prediction.values || [];
  ...
  const getTempColorClass = (tempValue) => {
    if (!tempValue && tempValue !== 0) return 'bg-gray-300';
    if (tempValue < 0) return 'bg-blue-500';
    if (tempValue < 15) return 'bg-blue-300';
    if (tempValue < 25) return 'bg-green-400';
    if (tempValue < 35) return 'bg-yellow-400';
    return 'bg-red-500';
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center">
          ...
          <p className="mt-3 max-w-2xl mx-auto text-xl text-gray-500 dark:text-gray-400 sm:mt-4">
            Monitor land surface temperature from satellite measurements.
          </p>
        </div>
          ...
          <div className="px-4 py-5 sm:p-6">
                ...
                <dl>
                  ...
                  <dd>
                    <div className="text-lg font-medium text-gray-900 dark:text-white">
                      The radiative skin temperature of the land
                    </div>
                  </dd>
                </dl>
              </div>
            ...
            <div className="mt-5">
              <p className="text-gray-600 dark:text-gray-300">
                Land Surface Temperature (LST) is how hot the "surface" of the Earth would feel to the touch in a particular location. It's not the same as air temperature (which is measured by weather stations) but rather the temperature of the actual land surface as measured by satellite sensors.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="mt-8 bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg">
            <div className="px-4 py-5 sm:p-6">
              ...
              {result.temperature !== undefined && (
                <div className="mb-6">
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Average Temperature:</p>
                  <div className="flex items-center">
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center text-white font-bold ${getTempColorClass(result.temperature)}`}>
                      {result.temperature.toFixed(1)}°C
                    </div>
                    <div className="ml-4">
                      <p className="text-gray-700 dark:text-gray-300">
                        {result.temperature < 0 ? 'Below freezing' :
                         result.temperature < 15 ? 'Cool conditions' :
                         result.temperature < 25 ? 'Mild conditions' :
                         result.temperature < 35 ? 'Warm conditions' :
                         'Hot conditions'}
                      </p>
                    </div>
                  </div>
              ...
              {chartData && (
                <div className="mt-6">
                  <h3 className="text-md font-medium text-gray-900 dark:text-white mb-2">Historical Temperature Data</h3>
                  <div className="h-80">
              ...
              {predictionData && (
                <div className="mt-8">
                  <h3 className="text-md font-medium text-gray-900 dark:text-white mb-2">Temperature Prediction (Next Week)</h3>
                  <div className="h-80">
                    <Line 
                      options={{
                        ...chartOptions,
